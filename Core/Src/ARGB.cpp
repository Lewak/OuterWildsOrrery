...
void ARGB_Flicker(uint16_t time, u8_t RGB[3], float percentage, int period_in_ms, int brightness)
{
	flickerTimer += time;
	//output = 255.0 - ((percentage/100.0) * abs(sin_taylor(2*3.14*(1000.0/period_in_ms)* flickerTimer/1000.0) * 255.0));
	//output = 255.0 - ((percentage/100.0) * 255.0 * (abs((flickerTimer % period_in_ms) - period_in_ms/2))/period_in_ms );
	int P = period_in_ms/2.0;
	float A = 255 * (percentage/100);
	output = (255 - ((A/P) * (P - abs(flickerTimer % (2*P) - P) ))) * brightness/255.0;

	//if(output > 255) output = 255;
	ARGB_SetBrightness(output);
	//ARGB_SetRGB(0,0,0,0);

	ARGB_SetRGB(1,RGB[0], RGB[1], RGB[2]);
}
...
void ARGB_SetRGB(u16_t i, u8_t r, u8_t g, u8_t b) {
    ...
    // set brightness
    r /= 256.0 / ((u16_t) ARGB_BR + 1);
    g /= 256.0 / ((u16_t) ARGB_BR + 1);
    b /= 256.0 / ((u16_t) ARGB_BR + 1);
